...
class TestOrderViaSuperUser:
    # GET
    def test_get_all_orders(self, client):
        response = client.get(
            f'/orders/', headers=superuser_token
        )
    ...
    def test_get_order_by_start_dt(self, start_dt, number_of_orders, client):
        response = client.get(
            f'/orders/?start_datetime={start_dt}', headers=superuser_token
        )
        assert response.status_code == 200
        assert 'application/json' in response.headers['Content-Type']
    ...
    def test_get_order_by_end_dt(self, end_dt, number_of_orders, client):
        response = client.get(
            f'/orders/?end_datetime={end_dt}', headers=superuser_token
        )
        assert response.status_code == 200
        assert 'application/json' in response.headers['Content-Type']
        assert len(response.json()) == number_of_orders
    ...
    def test_get_order_by_start_end_dt(self, start_dt, end_dt, number_of_orders, client):
        response = client.get(
            f'/orders/?start_datetime={start_dt}&end_datetime={end_dt}', headers=superuser_token
        )
        assert response.status_code == 200
        assert 'application/json' in response.headers['Content-Type']
        assert len(response.json()) == number_of_orders
    ...
    def test_get_order_by_status(self, status, number_of_orders, client):
        response = client.get(
            f'/orders/?status={status}', headers=superuser_token
        )
        assert response.status_code == 200
        assert 'application/json' in response.headers['Content-Type']
        assert len(response.json()) == number_of_orders
    ...
    def test_get_order_by_user_id(self, user_id, number_of_orders, client):
        response = client.get(
            f'/orders/?user_id={user_id}', headers=superuser_token
        )
        assert response.status_code == 200
        assert 'application/json' in response.headers['Content-Type']
        assert len(response.json()) == number_of_orders
    ...
    @pytest.mark.parametrize('order_id, result_msg, status', [
        (1, get_text('delete').format('order', 1), 200),
        (2, get_text('delete').format('order', 2), 200),
        (10, get_text('not_found').format('order', 10), 404)
    ])
    ...
    def test_patch_order_by_id(self, order_id, json_to_send, result_json, client):
        before_patch_response = client.get(
            f'/orders/{order_id}', headers=superuser_token
        )
        main_response = client.patch(
            f'/orders/{order_id}', json=json_to_send, headers=superuser_token
        )
    ...
    def test_post_order(self, json_to_send, result_json, client):
        ...
        main_response = client.post(
            '/orders/create', json=json_to_send, headers=superuser_token
        )
...
class TestOrderExceptionViaSuperuser:
    # PATCH
    ...
    def test_patch_wrong_order(self, order_id, json_to_send, result_json, status, client):
        for token in superuser_token, confirmed_client_token:
...
class TestOrderExceptionViaConfirmedUser:
    # PATCH
    @pytest.mark.parametrize("order_id, json_to_send, result_json, status", [
        # give equal fields start and end datetime
        (
